...
def etl_process(**kwargs):
    ...
    locallog = pd.DataFrame({'tipo':['recovered'], 'fecha':[datetime.now()]})
    ...
    with psql_connection.begin() as connection:
        connection.execute("truncate recovered")
        carga.to_sql('recovered', con=connection, if_exists='append', index=False)
...
dag = DAG('recovered', description='Load COVID recovered cases',
          default_args={
              'owner': 'grupo.dos',
